...
export default function AddMealDialog({
    ...
    const filteredRecipes = useMemo(() => {
        return recipes.filter((recipe) => {
            const title = (locale === 'de' ? recipe.titleDe : recipe.titleEn) || recipe.titleDe || recipe.titleEn || '';
    ...
    // Get recipe title with fallback
    const getRecipeTitle = (recipe: typeof recipes[0]) => {
        return locale === 'de'
            ? (recipe.titleDe || recipe.titleEn)
            : (recipe.titleEn || recipe.titleDe);
    };
        ...
        <Dialog open={open} onOpenChange={(open) => {
            ...
            <DialogContent className="sm:max-w-[700px] h-[80vh] flex flex-col p-0 overflow-hidden bg-zinc-900 border-white/10 text-white">
                <DialogHeader className="px-6 py-4 border-b border-white/5 flex-shrink-0">
                    <DialogTitle className="text-white">{t('selectRecipe')}</DialogTitle>
                </DialogHeader>
                        ...
                        <div className="flex-1 overflow-y-auto p-2 space-y-2">
                            {filteredRecipes.length === 0 ? (
                                <div className="flex flex-col items-center justify-center h-40 text-zinc-500">
                                    ...
                                    <p className="text-sm">{t('noRecipesFound')}</p>
                                </div>
                            ) : (
                                filteredRecipes.map((recipe) => {
                                    ...
                                    return (
                                        <div
                                            ...
                                            className={`flex items-center gap-3 p-2 rounded-lg cursor-pointer transition-colors ${isSelected ? 'bg-zinc-800 ring-1 ring-white/10' : 'hover:bg-zinc-800/50'
                                                }`}
                                        >
                                            <div className="w-12 h-12 rounded-md bg-zinc-800 overflow-hidden flex-shrink-0 relative">
                                                {recipe.imageUrl ? (
                                                    <Image src={recipe.imageUrl} alt={title || ''} fill className="object-cover" referrerPolicy="no-referrer" />
                                                ) : (
                        ...
                        <div className="flex flex-col w-full md:w-1/2 animate-in slide-in-from-right-10 duration-200 bg-zinc-900">
                            <div className="flex-1 p-6 overflow-y-auto">
                                ...
                                <h3 className="text-xl font-bold mb-6 text-white">
                                    {getRecipeTitle(activeRecipe)}
                                </h3>

                                <div className="space-y-4 bg-zinc-950 p-6 rounded-xl border border-white/5">
                                    <Label className="text-zinc-400 uppercase tracking-wider text-xs font-bold">{t('adjustServings')}</Label>
                                    ...
                                    <p className="text-xs text-zinc-500 text-right">
                                        {t('originalRecipe')}: {activeRecipe.servings || 2} {t('portions')}
                                    </p>
                                </div>
                            </div>
